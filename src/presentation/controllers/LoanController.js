// Controller responsável pelas operações de empréstimos (mock)
...
exports.createLoan = async (req, res) => {
  ...
  if (!dataPrevistaDevolucao) return res.status(400).json({ message: "dataPrevistaDevolucao é obrigatória" });
  if (!itens || !Array.isArray(itens) || itens.length === 0) {
    return res.status(400).json({ message: "É necessário informar ao menos um exemplar (itens)" });
  }
